...
把论点拆成六个部件：主张、数据、论据（Warrant）、支撑（Backing）、限定词、反驳条件。评委的追问几乎都在攻击 Warrant（"凭什么从这个数据得出这个结论？"）或 Backing（"这个方法本身合理吗？"）。脚本提前把 Warrant 说出来，减少被追问的入口。

**Hyland 元话语理论 — 措辞分寸**

学术可信度来自用词准确反映证据强度，而不是斩钉截铁。"suggests"和"demonstrates"不能混用；因果动词（leads to、improves）只在实验设计支持因果推断时使用，否则改为"is associated with"。

**Swales CARS 模型 — 开场结构**

建立领域 → 找到缺口 → 填补缺口。答辩开场的前 45 秒照这三步走，让评委在第一分钟就能定位你的研究贡献，而不是等你讲完方法才搞清楚"这个研究是干嘛的"。
